...
class FeedScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<FeedViewModel>(
      create: (_) => getIt<FeedViewModel>(), // Start listening on create
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              ...
              onPressed: () {
                // Logout is handled by a different VM, but we can access it or passing context
                // Ideally we use a provider for AuthViewModel too if needed here,
                // or just use getIt<AuthViewModel>().logout() but that won't rebuild the parent wrapper automatically
                // unless the wrapper is listening to the stream (which it is).
        ...
        body: Consumer<FeedViewModel>(
          builder: (context, viewModel, child) {
            ...
            return RefreshIndicator(
              onRefresh: () async {
                // The stream updates automatically, but we could force a refresh if logic changed.
                // For Firestore streams, pull-to-refresh isn't strictly necessary unless we are doing one-shot gets.
                // We'll leave it as a no-op or re-init logic if we change to Future-based.
              },
